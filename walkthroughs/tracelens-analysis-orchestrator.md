...
|---|---|---|
| Eager (training or eager inference) | Yes | Yes |
| Graph + capture (vLLM or SGLang) | Yes | No |
| Graph only | No | No |

For vLLM or SGLang inference traces, the canonical collection guide in
...
- Node.js 22 or later, required for the [`skills` CLI](https://github.com/vercel-labs/skills) (`npx skills ...`)
...
non-vLLM or SGLang eager inference).
...
|---|---|
| Platform | `MI300X` |
| Analysis mode | `default` (training or eager inference) |
| Environment | local, local+venv, cluster, or cluster+container |
| Node, container, or venv | only if running remotely |
...
- **Inference profiling:** Use the TraceLens profiling skill
  ...
  walkthrough to collect vLLM or SGLang traces before analysis.
