...
For this walkthrough, use [`danielholanda/dictate`](https://github.com/danielholanda/dictate),
...
**Hardware:** Any Windows x64 PC works. The skill selects a backend once at integration time based on your development machine. If your machine has a neural processing unit (NPU) and the chosen recipe supports it, the NPU backend is used. Otherwise it transparently falls back to Vulkan as the universal fallback so the app works on any end-user machine. The skill logs which backend was selected and why, so you always know what ran.

| Priority | Your hardware | What you get |
|---|---|---|
| 1 (preferred) | Ryzen AI with XDNA2 NPU (Strix, Strix Halo, Kraken, Gorgon Point) | NPU-accelerated transcription |
| 2 | AMD iGPU or dGPU | GPU-accelerated transcription |
...
## Step 2: Check which skills are available

Confirm that local AI integration skills are not yet visible.
...
## Step 3: Enable Claude to see `local-ai-app-integration`
...
2. Probe hardware (`GET /api/v1/system-info`) and pick the best available
   backend for `Whisper-Large-v3-Turbo`, NPU-first:
   - XDNA2 NPU present → whispercpp NPU backend
   - else AMD iGPU or dGPU → whispercpp iGPU or dGPU backend
...
> now?\n"` on quiet audio are a known Whisper behavior on silence or low-energy
> input, not an integration bug.

## Step 6: (Optional) Go beyond
...
* Model touching many files instead of flipping a single base-URL or key config object.
