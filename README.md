...
from it. Each source follows one branch: `main` by default, or the branch its
owners approved, such as a `release/*` pattern that follows the newest release.
Nothing reaches users that the owning team has not already merged there.
